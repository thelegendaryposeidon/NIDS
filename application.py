...
def ipInfo(addr=''):
    ...
    # Cascade through fast, free geolocation services
    for url, key in [
        (f'http://ip-api.com/json/{addr}?fields=countryCode,status', 'countryCode'),
        (f'https://ipwho.is/{addr}', 'country_code'),
        (f'https://ipinfo.io/{addr}/json', 'country')
    ]:
        try:
            ...
            with urlopen(req, timeout=1.5) as res:
                data = json.load(res)
                code = data.get(key)
                if code and code not in ['ano', 'unknown']:
                    _ip_country_cache[addr] = code
                    return code
        except Exception:
            continue

    _ip_country_cache[addr] = None
    return None
__author__ = 'hoang'
...
# Compatibility patch for unpickling legacy NumPy RandomState in models/explainer
import numpy.random._pickle as _np_pickle
_old_randomstate_ctor = _np_pickle.__randomstate_ctor
def _compat_randomstate_ctor(*args, **kwargs):
    if len(args) > 1:
        return _old_randomstate_ctor(args[0])
    return _old_randomstate_ctor(*args, **kwargs)
